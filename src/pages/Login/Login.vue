        ...
        <form @submit.prevent="login">
          ...
          <div :class="{on: !loginWay}">
            <section>
              ...
              <section class="login_message">
                <input type="text" maxlength="11" v-model="captchaCode" placeholder="验证码">
  ...
  methods: {
    ...
    async login () {
      ...
      if (this.loginWay) { // 短信登陆
        ...
        const {name, pwd, captchaId, captchaCode} = this
        if (!this.name) {
          // 用户名必须指定
        ...
        result = await reqPwdLogin(name, pwd, captchaId, captchaCode)
      }
